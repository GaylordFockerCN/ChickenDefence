...
void MoveControllerBase::listenerMonster(float t)
{
	...
	int State_Burn = MonsterState & 16;

	if (_pMonster->getFreezeCnt() < 20/*限制冻结次数以防永冻*/ && State_Stop && _pMonster->getfStopDuration() > 0)
	{
	...
	else if (State_Poison && _pMonster->getfPoisonDuration() > 0)
	{
		...
		atk._iAtk = 2;
		atk._iDuration = 0;
		atk._elementType = Element::Dendro;

		Sprite * pTemp = Sprite::create();
		pTemp->setPosition(_pMonster->getContentSize().width / 2, 40);
		_pMonster->getSprite()->addChild(pTemp);
		Animation * pAnimation;
		pAnimation = AnimationMaker(6, t);
		auto pCallFunc = CallFunc::create([=](){
			if (!_pMonster->getIsDead()) _pMonster->beHurt2(atk);
	...
	else if (State_Burn && _pMonster->getfBurnDuration() > 0)
	{
		float StateTime = _pMonster->getfBurnDuration() - t;
		if (StateTime < 0) StateTime = 0;
		_pMonster->setfPoisonDuration(StateTime);
		AtkProperty atk;
		atk._enAtkState = 1;
		...
		for (int i = 1; i < 10; i++)
		{
			...
			pAnimation->addSpriteFrame(SpriteFrameCache::getInstance()->getSpriteFrameByName(hurtSpriteName));
		}
		pAnimation->setDelayPerUnit(0.03);
		pAnimation->setLoops(1);
		auto pCallFunc = CallFunc::create([=](){
			if (!_pMonster->getIsDead()) _pMonster->beHurt2(atk);
			if (_pMonster->getfBurnDuration() <= 0)
			{
				_pMonster->setIState(0);
			}
			pTemp->removeFromParentAndCleanup(true);
		});
		pTemp->runAction(Sequence::create(Animate::create(pAnimation), pCallFunc, NULL));
