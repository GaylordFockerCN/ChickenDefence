...
void GameEndLayer::createMenu()
{
	...
	sp->setPosition(Vec2(VisibleRect::center()));
	sp->setScale(1.5f);
	addChild(sp);
	...
	if (_enGameEndType == en_GameLose) {
		SoundUtil::getInstance()->playEffect("ikun/Music/niganma.mp3");
		for (int i = 0; i <= 47; i++)
		{
			pAnimation->addSpriteFrame(cache->getSpriteFrameByName(StringUtils::format("loose%02d.png", i)));
			pAnimation->setDelayPerUnit(0.2f);
		}
	}
	else {
		SoundUtil::getInstance()->playEffect("ikun/Music/vote.mp3");
		for (int i = 0; i <= 52; i++)
		{
			pAnimation->addSpriteFrame(cache->getSpriteFrameByName(StringUtils::format("love%02d.png", i)));
	...
	auto se = Sequence::create(pAnimate, CallFunc::create([&]() {
